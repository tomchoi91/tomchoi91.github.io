...
holstein_by_region_plot <- ggplot(data=holstein_by_region, aes(x=region, y=b, fill=region)) + geom_bar(stat="identity") +
  scale_fill_brewer() +
  # theme_dark() +
...
jersey_by_region_plot <- ggplot(data=jersey_by_region, aes(x=region, y=b, fill=region)) + geom_bar(stat="identity") +
  scale_fill_brewer() +
  # theme_dark() +
...
crossbred_by_region_plot <- ggplot(data=crossbred_by_region, aes(x=region, y=b, fill=region)) + geom_bar(stat="identity") +
  scale_fill_brewer() +
  # theme_dark() +
...
milk_histrogram <- milk_analysis %>%
  ...
  geom_histogram(aes(color=region), binwidth = 1, position = "identity", alpha=.1) +
  geom_histogram(data=subset(milk_analysis,
                             (breed=="Holstein" & region =="NE")),
                 aes(fill=region), binwidth = 1, position = "identity", alpha=.5) +
  geom_histogram(data=subset(milk_analysis,
...
milk_by_state_viz <- plot_usmap(data = subset(milk_by_state, !is.na(state)), values = "b", labels = TRUE) +
...
cows_by_state <- milk_analysis %>%
  group_by(state) %>%
  summarise(a=round(sum(cows_in_milk, na.rm=TRUE),1),
            b=round(mean(milk_lbs, na.rm=TRUE),1),
            c=round(mean(fat_pct, na.rm=TRUE),1),
            d=round(mean(protein_pct, na.rm=TRUE),1),
            e=round(mean(fat_protein_lbs, na.rm=TRUE),1),
...
cows_by_state_viz <- plot_usmap(data = subset(cows_by_state, !is.na(state)), values = "a", labels = TRUE) +
